...
import { testMutations } from "circom-mutator";
...
testMutations({
  description: "pubkey_membership",
  filename: path.join(__dirname, "./circuits/pubkey_membership_test.circom"),
  test: async () => {
    // Compile the circuit
    const circuit = await wasm_tester(
      path.join(__dirname, "./circuits/pubkey_membership_test.circom"),
    ...
    expect(async () => await circuit.checkConstraints(w)).rejects.toThrow();
  }
});
